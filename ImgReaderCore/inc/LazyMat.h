...
namespace lazyimg
{
	...
	public:
		...
		// init image
		// eg : Mat(DataType(U8, C3), 1280, 720) -> 3 color channel 1280(w) x 720(h) empty image
		Mat(byte type, uint32_t w, uint32_t h);

		// eg : Mat(DataType(U8, C3), 1280, 720, <any*> ptr) -> 3 color channel 1280(w) x 720(h) image , The data from ptr
		Mat(byte type, uint32_t w, uint32_t h, const void* i_pdata);
